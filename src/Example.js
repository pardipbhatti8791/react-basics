...
function App() {
    ...
    const updateItem = (item, title) => {
        let textItem = input
        const filterText = textItem.replace(item.title)
        setInput(filterText)
    }
                    ...
                    {todos.length > 0 ? todos.map((item, i) => {
                        return (
                            <div key={i + 1} className="flex mb-4 items-center">
                                ...
                                <button onClick={() => setUpdate(item.id)} className="flex-no-shrink p-2 ml-2 border-2 rounded text-red border-red hover:text-white hover:bg-red">Update</button>
